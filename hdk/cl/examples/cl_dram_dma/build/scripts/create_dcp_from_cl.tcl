...
set bitDir    $CL_DIR/build/bitstreams
...
set_msg_config -severity "CRITICAL WARNING" -string "STATIC_SHELL_I" -suppress
set_msg_config -severity "WARNING"          -string "STATIC_SHELL_I" -suppress
...
### Strategy options
##################################################
# TODO: Updated to comment out URAM checking process
switch $strategy {
    "BASIC" {
        puts "BASIC strategy."
...
#Encrypt source code
# TODO: Updated the script so currently not encrypting any CL source code.
source encrypt.tcl

#Set the Device Type
# TODO: Updated the device to U200
...
if {$implement} {
   ...
   if {$link} {
      ...
      set_property SCOPED_TO_CELLS {CL_I} [get_files $CL_DIR/build/checkpoints/$synth_dcp]
      ...
      link_design -top $TOP -part [DEVICE_TYPE] -reconfig_partitions {CL_I}
   ...
   # Generate bitstream
   puts "\nAWS FPGA: ([clock format [clock seconds] -format %T]) - Running bitstream generation";
   impl_step write_bitstream $TOP
