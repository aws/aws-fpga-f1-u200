...
set dbg_bridge [get_debug_cores -filter {NAME=~CL_I/*CL_DEBUG_BRIDGE*}]
if {[llength $dbg_bridge]} {
   ...
   set dbg_cores [get_debug_cores -filter {NAME=~CL_I/*}]
   ...
   foreach cell $dbg_hub_cells {
      set dbg_reg [get_cells -quiet -hier -filter NAME=~CL_I/*xsdbm*/$cell]
      if [llength $dbg_reg] {
         foreach reg $dbg_reg {
