...
do
  ...
  aws_alveo_shell_dir=alveo/u200/$hdk_shell_version
  # Download the sha256
...
#  echo -n "Do you agree to AWS_F1_Shell_License_Agreement [Y/N]: "
#  read x
#  if [ "$x" != "Y" ]; then
#    echo "exiting hdk_setup"
#    return 2
#  fi
  debug_msg "curl -s https://$hdk_resources_s3_bucket.s3.amazonaws.com/$aws_alveo_shell_dir/$shell_file.sha256 -o $hdk_file.sha256"
  curl -s https://$hdk_resources_s3_bucket.s3.amazonaws.com/$aws_alveo_shell_dir/$shell_file.sha256 -o $hdk_file.sha256 || { err_msg "Failed to download HDK shell's $shell_file version from $aws_alveo_shell_dir/$shell_file.sha256 -o $hdk_file.sha256"; return 2; }
  if grep -q '<?xml version' $hdk_file.sha256; then
     err_msg "Failed to download HDK shell's $shell_file version from $aws_alveo_shell_dir/$shell_file.sha256"
     cat $hdk_file.sha256
     return 2
  fi
  ...
  if [ ! -e $hdk_file ]; then
    ...
    info_msg "Downloading latest HDK shell $shell_file from $aws_alveo_shell_dir/$shell_file"
    # Use curl instead of AWS CLI so that credentials aren't required.
    curl -s https://$hdk_resources_s3_bucket.s3.amazonaws.com/$aws_alveo_shell_dir/$shell_file -o $hdk_file || { err_msg "HDK shell checkpoint download failed"; return 2; }
  fi
